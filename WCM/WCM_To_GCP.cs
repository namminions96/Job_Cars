...
namespace Job_By_SAP
{
    ...
    public class WCM_To_GCP
    {
        ...
        public List<WcmGCPModels> OrderWcmToGCPAsync(string configWcm)
        {
            ...
            {
                DbsetWcm.Open();
                _logger.Information($"Exec:SP_GET_SELLOUT_PBLUE_SET");
                var ExcPblueSet = DbsetWcm.Query(WCM_Data.SP_GET_SELLOUT_PBLUE_SET(), commandType: CommandType.StoredProcedure, commandTimeout: timeout);
                var results = DbsetWcm.Query<TransTempGCP_WCM>(WCM_Data.Procedure_SaleOut(), new { Batch = dateTimeString }, commandType: CommandType.StoredProcedure, commandTimeout: timeout).ToList();
